...
{
    public abstract class SimpleNameIdManager<TEntity>: SimpleManager<TEntity> where TEntity: BaseEntity, INameId
    {
        private NameIdService<TEntity> _nameIdService;

        public SimpleNameIdManager(NameIdService<TEntity> service):base(service)
        {
